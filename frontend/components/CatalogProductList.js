...
import { Star, Heart, MessageCircle, Eye, Inbox, ChevronLeft, ChevronRight, FolderOpen, Search } from 'lucide-react';
...
export default function CatalogProductList({
    ...
    const fetchProducts = async (sort, page) => {
        ...
        try {
            ...
            if (searchQuery) {
                // Режим поиска
                res = await catalogAPI.search(searchQuery, params);
            } else if (isRootCatalog || !categorySlug) {
    ...
    // При поиске скрываем сайдбар
    const showSidebar = !searchQuery && children && children.length > 0;

    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            {/* Sidebar — дочерние категории (только не в поиске) */}
            {showSidebar && (
            ...
            <div className={showSidebar ? 'lg:col-span-3' : 'lg:col-span-4'}>
                ...
                {searchQuery && (
                    <div className="mb-6 flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-gray-500 text-sm">Результаты поиска по запросу</p>
                            <p className="font-bold text-gray-900 text-lg">&laquo;{searchQuery}&raquo;</p>
                        </div>
                    </div>
                )}
